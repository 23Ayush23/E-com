...
const Notification = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const response = await axios.get(`${import.meta.env.BACKEND_URL}/api/order/get-notification`);
  ...
  const removeNotification = async (id) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.BACKEND_URL}/api/order/remove-notification`, { id });
      if (response.data.success) {
